...
typealias RegistrarEntry<T> = HexdummyexampleRegistrar<T>.Entry<out T>

abstract class HexdummyexampleRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Hexdummyexample.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is HexdummyexampleRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
